    ...
    *end_pos = '\0';
    ...
    if (memcmp(mac, mwifi_addr_any, MWIFI_ADDR_LEN) == 0) {
        ...
        if (addr == NULL || cJSON_IsArray(addr) != true) {
            MDF_LOGE("addr == NULL || cJSON-IsArray(addr) != true --> GOTO EXIT");
            MDF_FREE(request);
            goto _exit;
        }
    ...
    if (type == NULL || cJSON_IsString(type) != true) {
        MDF_LOGE("TYPE IS NULL --> GOTO EXIT");
        MDF_FREE(request->addrs_list);
        MDF_FREE(request);
        goto _exit;
    }
    ...
    cJSON *subtype = cJSON_GetObjectItem(obj, "subtype");

    if (subtype == NULL) {
        MDF_LOGE("subtype --> normal behaviour");
        request->subtype = MDF_MALLOC(strlen("no_update"));
        assert(request->subtype != NULL);
        strcpy(request->subtype, "no_update");
        MDF_LOGE("AFTER STRCPY: %s\n",request->subtype);
    } else if (strcmp(subtype->valuestring, "update") == 0){
        MDF_LOGE("UPDATE\n");
        request->subtype = MDF_MALLOC(strlen(subtype->valuestring));
        assert(request->subtype != NULL);
        strcpy(request->subtype, subtype->valuestring);
        MDF_LOGE("AFTER STRCPY: %s\n",request->subtype);
    } else if (strcmp(subtype->valuestring, "update") != 0){
        MDF_LOGE("subtype --> normal behaviour_1");
        request->subtype = MDF_MALLOC(strlen(subtype->valuestring));
        assert(request->subtype != NULL);
        strcpy(request->subtype, subtype->valuestring);
        MDF_LOGE("AFTER STRCPY: %s\n",request->subtype);
    ...
    if (strcmp(type->valuestring, "bytes") == 0) {
        if (cJSON_IsString(data) != true) {
            MDF_LOGE("Data should be string type");
            MDF_FREE(request->addrs_list);
            MDF_FREE(request);
            goto _exit;
        }
