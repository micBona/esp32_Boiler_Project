...
static const char *TAG = "mqtt_Node_GFE_Prov";
...
static void node_write_task(void *arg)
{
    ...
    for (;;) {
        ...
        size = asprintf(&data, "{\"Air_Temperature_Sensor\":%f, \"self\": \"%02x%02x%02x%02x%02x%02x\", \"parent\":\"%02x%02x%02x%02x%02x%02x\",\"layer\":%d,\"Root\":%d}",
                        pct2075d_read_temperature(72), MAC2STR(sta_mac), MAC2STR(parent_mac.addr), esp_mesh_get_layer(), esp_mesh_is_root());
